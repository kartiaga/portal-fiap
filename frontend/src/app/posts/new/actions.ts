...
export async function createPostAction(
  ...
  if (cleanTitle.length > 255)
    return { error: "O título deve ter no máximo 255 caracteres." };
  ...
  try {
    ...
    if (!response.ok) {
      const body: { message?: string } | null = await response
        .json()
        .catch(() => null);
      return { error: body?.message ?? "Não foi possível criar a publicação." };
    }
  } catch {
    return {
  ...
  redirect("/posts?feedback=created");
}
