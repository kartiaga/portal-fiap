...
describe('DTO schema validation', () => {
    ...
    it('rejects invalid createUser payloads', () => {
        expect(createUserSchema.safeParse({ email: 'bad', password: 'short', name: '' }).success).toBe(false)
    })

    it('validates login payload', () => {
        expect(loginSchema.safeParse({ email: 'test@example.com', password: 'securepass' }).success).toBe(true)
    })

    it('rejects invalid login payloads', () => {
        expect(loginSchema.safeParse({ email: 'test', password: 'short' }).success).toBe(false)
    })
    ...
    it('rejects invalid createPost payloads', () => {
        expect(createPostSchema.safeParse({ title: 'No', content: 'Too short' }).success).toBe(false)
    })

    it('validates createProfile payload', () => {
        expect(
            createProfileSchema.safeParse({ userId: 'user-1', name: 'Profile name' }).success,
        ).toBe(true)
    })

    it('rejects invalid createProfile payloads', () => {
        expect(createProfileSchema.safeParse({ userId: 'user-1' }).success).toBe(false)
    })
})
