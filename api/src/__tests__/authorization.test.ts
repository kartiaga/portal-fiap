...
import { requireAdmin, requireRole, requireTeacherOrAdmin } from '@/lib/authorization'
import { UserRole } from '@/modules/users/entities/user'

describe('authorization middleware', () => {
    const makeReply = () => ({ status: jest.fn().mockReturnThis(), send: jest.fn() })

    it('allows a user with the allowed role', async () => {
        const request = { user: { role: UserRole.ADMIN } } as any
    ...
    it('denies a user with an unauthorized role', async () => {
        const request = { user: { role: UserRole.STUDENT } } as any
        const reply = makeReply() as any
    ...
    it('allows teacher or admin roles', async () => {
        const reply = makeReply() as any
        await requireTeacherOrAdmin({ user: { role: UserRole.TEACHER } } as any, reply)
        expect(reply.status).not.toHaveBeenCalled()
    })
